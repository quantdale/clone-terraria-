...
| netproto.js | `TC.NetProto` (W22; v2 in W23; v3 in W24): versioned authoritative-multiplayer protocol — envelope `{v,t,sid,pid,cseq,sseq,tick,p}`, strict fail-closed validation incl. per-command bounded ctx schemas, message TYPES, COMMAND_WHITELIST, region full/delta codecs (v3 full lines carry tiles+walls+liquid type+amount hex layers; deltas are `[cellIdx,tile,wall,liqType,liqAmt]` quintuples), clean old-version rejection, deterministic state digests; transport-agnostic, headless-safe |
