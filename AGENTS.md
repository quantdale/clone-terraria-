...
| savecore.js | `TC.SaveCore`: versioned envelope {formatVersion:2, gameVersion, generationVersion, registryFingerprint, packs, metadata, world{}, character{}, systems{}}; provider registry `register('section.key',{serialize,deserialize,version})`; migrations; atomic saveNow (tmp→bak→swap); loadFrom with .bak + legacy-blob fallback; export/import; W25: 'packs' metadata written from TC.Packs.saveMetadata() and structurally validated on load |
| packs.js | `TC.Packs` (W25): THE pack authority — provide/provideJSON (fail-closed untrusted-input boundary: prototype-pollution keys, functions/symbols, non-finite numbers, oversize/depth caps, unknown fields, reserved namespaces, traversal paths all rejected before schema checks), declarative data families tiles/items/enemies/recipes with built-in-only references (ai/look/pattern/station) and commit-time normalization to canonical runtime forms (bare keys / numeric world ids), resource locale fragments via Localization.extend, dependency topo-order + version/game ranges + cycles, deterministic digests `digest()` (gameplay) / `contentDigest()` (all), ATOMIC journaled activation with session-permanence (`setActive/deactivateAll/bootActivate`), save classification `classifySave`, stats. Fixture: packs/testpack.js. No eval/new Function/script execution anywhere |
...
| netserver.js | `TC.NetServer.create(opts)` (W22/W23; W24 liquid layers): authoritative session runner — start/attachLocal/connect/tick/stop/runForever(summary); per-tick inbound processing → Runtime.tick → replication; private per-client WorldRegions consumers ('net:<cid>'), region interest (~56 tiles, configurable), baselined ENTITY deltas with rm tombstones + keyframe recovery at configurable cadence, dirty-region nearest-player priority, per-tick outbound byte budget, idle suppression; region baselines carry tiles+walls+liquid type/amount read via TC.Liquids.snapshotRegion (v3 lines); container sessions bound by InteractTile; detached-identity reconnect with cseq floors and configurable grace; stats in F3 |
