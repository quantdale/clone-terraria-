...
| enemydefs.js | Enemy content extensions extracted from constants-adjacent ownership (W13): ENEMY_DEFS biome regulars + bosses (king_slime, skeletron+hand, wof, storm_jelly, moss_mother), summon ITEM_DEFS (slime_crown/skull_sigil/flesh_sigil/blood_sigil), their RECIPES; pure data merged at load time |
| enemyai.js | `TC.EnemyAI` (W13): reusable behavior archetypes keyed by def.ai — slime/zombie/eye/bat/walker(+lunge/charge)/harpy/stationary/teleporter + boss AIs king_slime/eye_boss/skeletron/skele_hand/wof/storm_jelly/moss_mother; `get(aiName)→impl`, shared `util.{rand,clamp,approach,daylight,weightedPick,solidAt,rectSolid}`, `isFlyer(ai)` authority; impls `(e,{clock},dt)→bool` (false = remove) |
