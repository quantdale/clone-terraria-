...
| worldgen.js | `TC.WorldGen.generate(seed)` → `{width, height, tiles:Uint8Array, walls:Uint8Array, surfaceY:Int16Array, spawnX, spawnY, timings}`. Deterministic from seed + GENERATION_VERSION (never `Math.random`): structured as named passes `PASSES = terrain → surface-biomes → caves → ores → structures → decor → validation`, each with its own seeded rng stream (`runPass(name, ctx)` for debug); `CONFIG = {deepCaves, microBiomes, richOres}` feature flags (default off). Walls: WALL.DIRT/STONE background wherever `y > surfaceY[x]+1`; biomes snow/jungle/desert/ocean/corruption/dungeon/pyramids/underworld per GEN. Loot post-pass is invoked explicitly via `TC.Loot.populateWorld(gen, seed)` from main.buildWorld |
...
| npcs.js | `TC.NPCs`: `list[]`, `spawnGuide(x,y)` + generic `spawn(type,x,y)` over NPC_KINDS table (dialog/unlocks/shop/look), `evaluateUnlocks()` (consults TC.Progression + population), `validateHome(tx,ty,w,h)`, damage→respawn-at-home after delay; `update/draw/clear/serialize/load` (loader tolerates legacy blobs); `shopOf(type)/kindDef(type)` for UI |
...
| progression.js | `TC.Progression`: world flag store `set/has/all/resetForNewWorld`, FLAGS (boss.*.defeated, event.blood_moon.completed); auto-records BossDefeated; SaveCore provider 'systems.core.progression'; `spawnMultiplier()` scales enemy spawn rate per defeated boss |
| liquids.js | `TC.Liquids`: independent liquid layer foundation (type Uint8 + amount Uint8 arrays, active-cell budgeted settling, evaporation, water×lava→stone contact rule); `init/reset/update/wake/set/sampleAt/columnSurface/draw/importFromWorld/stats`; SaveCore provider 'world.core.liquids'; runs parallel to legacy WATER tiles until migration completes |
| debug.js | `TC.Debug`: rolling timings (mark/endMark/frame/stats), counters/snapshot, F3 overlay `drawHud` (fps buckets, liquids, projectiles, flags), `window.__TEST__` hooks only under location.hash '#test' |
