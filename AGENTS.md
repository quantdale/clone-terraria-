...
| netproto.js | `TC.NetProto` (W22; v2 in W23): versioned authoritative-multiplayer protocol — envelope `{v,t,sid,pid,cseq,sseq,tick,p}`, strict fail-closed validation incl. per-command bounded ctx schemas, message TYPES, COMMAND_WHITELIST (adds CraftRecipe/ShopBuy/ShopSell/ContainerMove), region full/delta codecs, deterministic state digests; transport-agnostic, headless-safe |
...
| netserver.js | `TC.NetServer.create(opts)` (W22/W23): authoritative session runner — start/attachLocal/connect/tick/stop/runForever(summary); per-tick inbound processing → Runtime.tick → replication; private per-client WorldRegions consumers ('net:<cid>'), region interest (~56 tiles, configurable), baselined ENTITY deltas with rm tombstones + keyframe recovery at configurable cadence, dirty-region nearest-player priority, per-tick outbound byte budget, idle suppression; container sessions bound by InteractTile; detached-identity reconnect with cseq floors and configurable grace; stats in F3 |
| netclient.js | `TC.NetClient.create(opts)` (W22/W23): joining-side controller — connect/sendCmd/sendIntent/disconnect/tryReconnect/frame; phases idle→connecting→syncing→playing→closed; applies snapshots/deltas into a presentation mirror (world tiles/walls via setRaw, player/enemy/drop mirrors with presence-based merge + tombstones, self-inventory + chest refresh); remote entities render through interpolation buffers and SELF locomotion predicts through canonical Player.update with bounded reconciliation (`interp`/`predict` opts); samples+streams input frames; never runs the local simulation while joined (`drivesTick()` gates main.js) |
