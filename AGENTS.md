...
| gamerng.js | `TC.GameRng` (W23): THE deterministic gameplay-runtime RNG — named streams 'ai'/'spawn'/'loot'/'combat'/'misc', reset on WorldLoaded from TC.worldSeed; `stream(name).float/range/int/chance/pick/sign`, `state/restore/digest` for replay proofs, `override/clearOverrides` test seam. All gameplay-affecting runtime randomness MUST route through it; presentation-only randomness and worldgen stay out |
...
| enemyspawn.js | `TC.EnemySpawn` (W26): spawn director + rules — `update(dt)` (Blood Moon dusk/dawn lifecycle then director tick), `spawnDirector`, `zoneOf(p,w,dl)→'day'|'night'|'cave'|'underworld'` (depth-first: underworld membership via TC.Biomes.isUnderworldAt outranks the generic cave rule; Blood Moon stays surface-night-only), `zoneTable(zone,pcol)` (biome override replaces vanilla for every zone except ordinary 'cave'; `[type,weight,condition]` entries filtered everywhere through Progression.test, fail-closed; **W26 pack spawnRules** merged deterministically via `TC.Packs.getSpawnRules()` seam — compiled at activation, indexed by zone, filtered by zone/biome/depth/time/requires, deterministic GameRng spawn stream), `findSpot(def,tx,ty,surf)`, `surfaceBiome/playerDepthT`, `setBloodMoon/isBloodMoon/reset` |
...
| wiring.js | `TC.Wiring`: wire tiles + mechanisms (switch/lever/pressure plate/timer/dart trap/actuator/inlet+outlet pump); BFS signal flood from a source cell, receivers fire once per pulse emitting WirePulse; pump endpoints reached by the same pulse process ONCE as an ascending-index deterministic batch through `TC.Liquids.set()` — exact volume/type conservation, `PUMP_ENDPOINT_CAP`/`PUMP_TRANSFER` bounded, counters via `pumpStats()`. Mechanism player semantics (plates, door-close safety, trap-dart victims) enumerate ALL registered players via the internal roster helper — never primary-only. No core patches: reacts to TileChanged events, exposes `isGhost(tx,ty)` consulted by World.isSolid, darts ride the projectile pool; `init/update/draw/pulse/toggleDevice/toggleTimer/placeWire/removeWire/interact/isGhost/attachActuatorAt/iconFor/pumpStats/serialize/load/reset/resetForNewWorld`; SaveCore provider 'systems.core.wiring' |
...
| commands.js | `TC.Commands`: canonical transactions `submit(name,ctx)→{ok,result}|{ok:false,error}` — MineTile/MineWall/PlaceTile/PlaceWall/UseItem (full live dispatch incl. integration hooks, hammer shaping, actuator, honest used/reason results)/MoveItem/EquipItem/CraftRecipe/InteractTile/ShopBuy/ShopSell + `ContainerMove` (W23: player↔chest transfer, server-bound session, authoritative auto-placement); PLUS deterministic FIFO queue: `enqueue/drain/pending/clearQueue/stats` drained once per tick in the scheduler's commands phase (snapshot semantics, bounded 256, cleared on WorldLoaded) |
| savecore.js | `TC.SaveCore`: versioned envelope {formatVersion:2, gameVersion, generationVersion, registryFingerprint, packs, metadata, world{}, character{}, systems{}}; provider registry `register('section.key',{serialize,deserialize,version})`; migrations; atomic saveNow (tmp→bak→swap); loadFrom with .bak + legacy-blob fallback; export/import; pack metadata accepts legacy gv1 and ordered gv2 shapes from TC.Packs.saveMetadata() |
| packs.js | `TC.Packs` (W25): THE pack authority — provide/provideJSON (fail-closed untrusted-input boundary: prototype-pollution keys, functions/symbols, non-finite numbers, oversize/depth caps, unknown fields, reserved namespaces, traversal paths all rejected before schema checks), declarative families tiles/items/enemies/recipes/walls/lootTables/spawnRules with built-in-only references and commit-time normalization, embedded resource locale fragments plus explicit `validateInstallJSON` rejection of unmaterialized `resources.files`, dependency topo-order + canonical version/game ranges + cycles, deterministic `digest()` (ordered gameplay identity) / `contentDigest()` (all content), `gameplayFingerprintMatches()` for verified W25 v4 migration, ATOMIC journaled activation with session-permanence, ordered gv2 save metadata plus legacy classification, stats. No eval/new Function/script execution anywhere |
| packstore.js | `TC.PackStore` (W26): durable installed-manifest store — versioned localStorage envelope `tc_packs_installed_v1` (v1) with per-manifest/total caps, corruption-safe degrade, install/remove/export/list/has/get/repair/loadErrors/stats; validates only installable source through `TC.Packs.validateInstallJSON` before persistence/provision, skips unmaterializable stored entries with bounded diagnostics, and preserves session permanence (active-pack guard on remove/replace) |
| targeting.js | `TC.Targets` (W23): THE deterministic multi-player target-selection policy — `of(e)/nearest(x,y)/all()/count()/anchor()`; eligible = registered live players (empty-registry single-player falls back to the legacy singleton), nearest-by-d2 with stable-id tie-break, per-entity stickiness (challenger must be ≥20% closer). Consumed by ALL enemy AI/despawn/spawn-anchor paths; camera/input/HUD/self-mirror stay deliberately primary/local |
| netproto.js | `TC.NetProto` (W22; v2 in W23; v3 in W24; v4 in W25): versioned authoritative-multiplayer protocol — envelope `{v,t,sid,pid,cseq,sseq,tick,p}`, strict fail-closed validation incl. per-command bounded ctx schemas, message TYPES, COMMAND_WHITELIST, region full/delta codecs (v3 full lines carry tiles+walls+liquid type+amount hex layers; deltas are `[cellIdx,tile,wall,liqType,liqAmt]` quintuples), v4 hello/welcome REQUIRE bounded packs meta `{fp,list}` proving gameplay pack-set identity before any join/rejoin binds, clean old-version rejection, deterministic state digests; transport-agnostic, headless-safe |
| nettransport.js | `TC.NetTransport` (W22/W23): byte-mover boundary — `loopbackPair()` deterministic duplex (manual pump + dropNext/dupNext/swapNext injection), `impairedPair(opts)` virtual-time latency/jitter/drop/dup/reorder/stall harness (seeded, ordered delivery like real transports), and `websocket(url)` platform client wrapper. Endpoints: send/onMessage/onStatus/close |
...
| netserver.js | `TC.NetServer.create(opts)` (W22/W23; W24 liquid layers): authoritative session runner — start/attachLocal/connect/tick/stop/runForever(summary); per-tick inbound processing → Runtime.tick → replication; v4 pack admission uses current ordered identity or a verified W25 legacy identity whose declared data order matches; private per-client WorldRegions consumers ('net:<cid>'), region interest (~56 tiles, configurable), baselined ENTITY deltas with rm tombstones + keyframe recovery at configurable cadence, dirty-region nearest-player priority, per-tick outbound byte budget, idle suppression; region baselines carry tiles+walls+liquid type/amount read via TC.Liquids.snapshotRegion; container sessions bound by InteractTile; detached-identity reconnect with cseq floors and configurable grace; stats in F3 |
| netclient.js | `TC.NetClient.create(opts)` (W22/W23; W24 liquid mirror): joining-side controller — connect/sendCmd/sendIntent/disconnect/tryReconnect/frame; phases idle→connecting→syncing→playing→closed; applies snapshots/deltas into a presentation mirror (world tiles/walls via setRaw, LIQUID truth via presentation-only TC.Liquids.applyMirrorRegion — no local settle/pump/reaction sim and no echo, player/enemy/drop mirrors with presence-based merge + tombstones, self-inventory + chest refresh); remote entities render through interpolation buffers and SELF locomotion predicts through canonical Player.update with bounded reconciliation (`interp`/`predict` opts); samples+streams input frames; never runs the local simulation while joined (`drivesTick()` gates main.js) |
...
| liquids.js | `TC.Liquids`: THE single runtime liquid authority (W1 migration — legacy WATER/LAVA tiles are worldgen/legacy-save representation only, imported into the volume layer at build time by main.buildWorld; no tile-water simulation exists). Type Uint8 + amount Uint8 arrays (water/lava/honey), budgeted settling with equalize/evaporation, water×lava→stone contact; every mutation seam (set/collectAt/placeAt/settling) reports to TC.WorldRegions. W24 replication seams: `snapshotRegion(baseX,baseY,size)` read-only layer copy (host), `applyMirrorRegion(...)` presentation-only joined-client writer (no settle/events/echo; marks local regions for repaint), `digest()` FNV over both layers; `init/reset/update/wake/set/sampleAt/queryAt/displace/collectAt/placeAt/onUseHeld/columnSurface/draw/importFromWorld/stats/mode/isLiquid`; SaveCore provider 'world.core.liquids'; buckets are kind-'bucket' items converting in place |
...
- **W23 multiplayer rules:** authoritative gameplay code must never read the
  ...
  New client-callable mutations need a protocol whitelist entry + bounded ctx
  schema + server-side eligibility checks; clients never declare truth.
- **W25 pack rules:** ALL pack content flows through `TC.Packs` — never append
  to TILE_DEFS/ITEM_DEFS/ENEMY_DEFS/RECIPES or call Registry.define directly
  from outside packs.js activation. Packs are declarative data only: no eval,
  no new Function, no callbacks/hooks from pack input, and the 'core' namespace
  is unreachable to them. Committed content is session-permanent; changing the
  active set means restart (Settings-backed, title panel). Saves carry the pack
  metadata and continueGame classifies BEFORE mutation — never weaken that gate.
  Gameplay pack-set identity rides protocol v4 hello/welcome; do not bypass it.
...
traps/actuators/inlet+outlet pumps), breakable pots + life crystals + chest loot, biome-aware soundtrack +
