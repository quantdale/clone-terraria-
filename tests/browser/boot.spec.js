...
test.describe('boot gate', () => {
  test('title screen boots clean: every script loads, no console/page errors', async ({ page }) => {
    ...
    const api = await page.evaluate(() => ({
      ...
      save: !!window.TC.Save, player: !!window.TC.Player, ui: !!window.TC.UI,
      enemies: !!window.TC.Enemies, registry: !!window.TC.Registry,
      savecore: !!window.TC.SaveCore, systems: !!window.TC.Systems,
      liquids: !!window.TC.Liquids, wiring: !!window.TC.Wiring,
      fishing: !!window.TC.Fishing, magic: !!window.TC.Magic,
  ...
  test('registry validates cleanly after FULL script load (wiring included)', async ({ page }) => {
    const errors = await H.openGame(page);
    const reg = await page.evaluate(() => {
      try {
        window.TC.Registry.validate();
        let count = 0;
        for (const k of window.TC.Registry.KINDS) count += window.TC.Registry.count(k);
        return { ok: true, fingerprint: window.TC.Registry.fingerprint(), count: count };
      } catch (e) {
        return { ok: false, error: String(e.message) };
      }
    });
    expect(reg.ok, 'post-boot Registry.validate() must pass: ' + (reg.error || '')).toBe(true);
    expect(reg.count).toBeGreaterThan(100);
    H.assertNoErrors(errors, 'boot/registry');
  });

  test('no duplicate system / render-layer / save-provider registrations at boot', async ({ page }) => {
    const errors = await H.openGame(page);
    const dupes = await page.evaluate(() => {
      const out = { systems: [], layers: [], providers: [] };
      const countBy = (arr) => {
        const m = new Map();
        for (const x of arr) m.set(x, (m.get(x) || 0) + 1);
        return [...m.entries()].filter(([, n]) => n > 1).map(([k]) => k);
      };
      if (window.TC.Systems && window.TC.Systems.names) out.systems = countBy(window.TC.Systems.names());
      if (window.TC.RenderLayers && window.TC.RenderLayers.names) out.layers = countBy(window.TC.RenderLayers.names());
      if (window.TC.SaveCore && window.TC.SaveCore.providerNames) out.providers = countBy(window.TC.SaveCore.providerNames());
      return out;
    });
    expect(dupes.systems).toEqual([]);
    expect(dupes.layers).toEqual([]);
    expect(dupes.providers).toEqual([]);
    H.assertNoErrors(errors, 'boot/dupes');
  });

  test('new world via __TEST__ hook renders animated frames with zero errors', async ({ page }) => {
    ...
    expect(anim.framesAdvanced, 'draw loop must keep executing frames').toBeGreaterThan(3);
    expect(anim.distinctColors, 'framebuffer must hold varied content, not blank').toBeGreaterThan(4);
