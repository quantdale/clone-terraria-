...
(function () {
  ...
  if (TC.ENEMY_DEFS) {
    Object.assign(TC.ENEMY_DEFS, {
      // ---- regular enemies ----
      harpy:         { name: 'Harpy',          hp: 60,  dmg: 16, kbResist: 0.4,  ai: 'harpy',  w: 30, h: 24,
                       color: '#b8a06a', drops: [ { id: 'feather', min: 1, max: 2, chance: 0.9 } ] },
      vulture:       { name: 'Vulture',        hp: 40,  dmg: 13, kbResist: 0.3,  ai: 'bat',    w: 24, h: 18,
                       color: '#9a7a52', drops: [ { id: 'feather', min: 1, max: 1, chance: 0.6 } ] },
      eater_of_souls:{ name: 'Eater of Souls', hp: 70,  dmg: 20, kbResist: 0.45, ai: 'eye',    w: 30, h: 26,
                       color: '#7a4a9a', drops: [ { id: 'shadow_shard', min: 1, max: 1, chance: 0.5 } ] },
      ice_slime:     { name: 'Ice Slime',      hp: 30,  dmg: 14, kbResist: 0.3,  ai: 'slime',  w: 32, h: 20,
                       color: '#9adcf0', drops: [ { id: 'gel', min: 1, max: 2, chance: 1 } ] },
      sand_slime:    { name: 'Sand Slime',     hp: 42,  dmg: 15, kbResist: 0.3,  ai: 'slime',  w: 34, h: 22,
                       color: '#d8c86e', drops: [ { id: 'gel', min: 1, max: 3, chance: 1 } ] },
      jungle_bat:    { name: 'Jungle Bat',     hp: 26,  dmg: 14, kbResist: 0.2,  ai: 'bat',    w: 20, h: 16,
                       color: '#4a8a3a', drops: [] },
      skeleton:      { name: 'Skeleton',       hp: 60,  dmg: 18, kbResist: 0.55, ai: 'walker', w: 20, h: 40,
                       color: '#cfc8b8', speed: 55, jumpVel: 330, look: 'skeleton',
                       drops: [ { id: 'bone', min: 1, max: 2, chance: 1 } ] },
      granite_golem: { name: 'Granite Golem',  hp: 120, dmg: 26, kbResist: 0.85, ai: 'walker', w: 28, h: 38,
                       color: '#7a8a96', speed: 34, jumpVel: 300, look: 'golem', defense: 4,
                       drops: [ { id: 'granite_shard', min: 1, max: 2, chance: 0.8 } ] },
      blood_crawler: { name: 'Blood Crawler',  hp: 50,  dmg: 20, kbResist: 0.4,  ai: 'walker', w: 28, h: 18,
                       color: '#a02a2a', speed: 95, jumpVel: 380, look: 'crawler', bloodMoonOnly: true,
                       drops: [ { id: 'blood_shard', min: 1, max: 2, chance: 0.7 } ] },
      crimson_slime: { name: 'Crimson Slime',  hp: 58,  dmg: 19, kbResist: 0.4,  ai: 'slime',  w: 36, h: 24,
                       color: '#c42a3a', bloodMoonOnly: true,
                       drops: [ { id: 'gel', min: 2, max: 4, chance: 1 }, { id: 'blood_shard', min: 1, max: 1, chance: 0.5 } ] },
      hungry:        { name: 'Hungry',         hp: 36,  dmg: 22, kbResist: 0.2,  ai: 'eye',    w: 22, h: 22,
                       color: '#b04a4a', drops: [] },   // Wall of Flesh servant

      // ---- bosses (all kbResist 1; respect MAX_BOSSES via spawnBoss) ----
      king_slime:    { name: 'King Slime',     hp: 900,  dmg: 26, kbResist: 1, ai: 'king_slime', w: 80, h: 56,
                       color: '#4a6fd6', boss: true, defense: 6,
                       drops: [ { id: 'gel', min: 30, max: 50, chance: 1 },
                                { id: 'gold_bar', min: 4, max: 7, chance: 1 },
                                { id: 'slime_crown', min: 1, max: 1, chance: 0.5 } ] },
      skeletron:     { name: 'Skeletron',      hp: 1600, dmg: 30, kbResist: 1, ai: 'skeletron',  w: 64, h: 64,
                       color: '#ddd6c2', boss: true, defense: 10,
                       drops: [ { id: 'bone', min: 20, max: 35, chance: 1 },
                                { id: 'gold_bar', min: 6, max: 10, chance: 1 } ] },
      skele_hand:    { name: 'Skeletron Hand', hp: 280,  dmg: 24, kbResist: 1, ai: 'skele_hand', w: 26, h: 38,
                       color: '#cfc8b8', part: true, drops: [ { id: 'bone', min: 3, max: 6, chance: 1 } ] },
      wof:           { name: 'Wall of Flesh',  hp: 2400, dmg: 34, kbResist: 1, ai: 'wof',        w: 46, h: 170,
                       color: '#c43a3a', boss: true, defense: 12,
                       drops: [ { id: 'blood_shard', min: 15, max: 25, chance: 1 },
  ...
  if (TC.ITEM_DEFS) {
    Object.assign(TC.ITEM_DEFS, {
      ...
      slime_crown:   { name: 'Slime Crown', kind: 'summon', maxStack: 20, boss: 'king_slime' },
      skull_sigil:   { name: 'Skull Sigil', kind: 'summon', maxStack: 20, boss: 'skeletron' },
      flesh_sigil:   { name: 'Flesh Sigil', kind: 'summon', maxStack: 20, boss: 'wof' },
      blood_sigil:   { name: 'Blood Sigil', kind: 'summon', maxStack: 20, boss: '__blood_moon__' }
    });
  }
  ...
  if (TC.RECIPES && typeof TC.RECIPES.push === 'function') {
    TC.RECIPES.push(
      { out: 'slime_crown', n: 1, station: 'anvil',     cost: { gel: 20, gold_bar: 3 } },
      { out: 'skull_sigil', n: 1, station: 'anvil',     cost: { bone: 15, gold_bar: 3 } },
      { out: 'flesh_sigil', n: 1, station: 'anvil',     cost: { blood_shard: 12, shadow_shard: 6, iron_bar: 5 } },
      { out: 'blood_sigil', n: 1, station: 'workbench', cost: { blood_shard: 5 } }
    );
  }

  const FLASH_TIME = 0.15;
  const TOUCH_KB_X = 210, TOUCH_KB_Y = -190;
  ...
  function daylight() {
    return (TC.Sky && typeof TC.Sky.daylight === 'function') ? TC.Sky.daylight() : 1;
  }
  ...
  const EXTRA_SPAWN = {
    day:   [ ['harpy', 0.7], ['vulture', 0.6], ['ice_slime', 1.2], ['sand_slime', 1.0] ],
    night: [ ['eater_of_souls', 0.8] ],
    cave:  [ ['skeleton', 1.4], ['granite_golem', 0.7], ['jungle_bat', 0.8] ]
  };
  const BLOOD_MOON_TABLE = [
    ['zombie', 3], ['demon_eye', 2],
    ['blood_crawler', 2.5], ['crimson_slime', 2], ['eater_of_souls', 1]
  ];
  ...
  function isFlyer(ai) {
    return ai === 'eye' || ai === 'bat' || ai === 'eye_boss' ||
           ai === 'harpy' || ai === 'skeletron' || ai === 'skele_hand' ||
           ai === 'hungry' || ai === 'wof';
  }
  ...
  function moveAndCollide(e, dt) {
    ...
    const bounce = flyer ? (e.def.ai === 'bat' ? 0.6 : e.def.ai === 'eye_boss' ? 0.4 : 0.5) : 0;
    ...
    const maxX = w.width * ts - e.w, maxY = w.height * ts - e.h;
  ...
  function runAI(e, dt) {
    ...
    switch (e.def.ai) {
      ...
      case 'bat': {
        e.phase += dt * 5.5;
        e.jitterTimer -= dt;
        if (e.jitterTimer <= 0) {          // sudden erratic course change
        ...
        if (p && !p.dead) {
          ...
          const gy = pcy + Math.sin(oa) * orbR * 0.6 + Math.sin(e.phase * 1.7) * 14;
        ...
        if (bsp > 140) { e.vx *= 140 / bsp; e.vy *= 140 / bsp; }
        e.facing = e.vx >= 0 ? 1 : -1;
        return true;
      }

      case 'eye_boss': {
        if (!e.phase2 && e.hp <= e.maxHp * 0.5) {   // rage at half health
          e.phase2 = true;
          e.bstate = 'hover';
          e.dashTimer = Math.max(e.dashTimer, 1.2);
          if (TC.Particles) {
            TC.Particles.burst(ecx, ecy, 26, {
              colors: [e.def.color, '#ff3040'], speed: 170, life: 0.7, size: 3, gravity: 0
            });
          }
        }
        ...
        const hy = pcy - (e.hoverH || 14 * TC.CONST.TS) +
                   Math.sin(clock * 1.05 + e.phase) * 12 - e.h / 2;
        ...
        if (hsp > hmax) { e.vx *= hmax / hsp; e.vy *= hmax / hsp; }
        ...
        if (e.phase2 && p && !p.dead) {
          ...
          if (e.summonTimer <= 0) {
            ...
            for (let k = 0; k < n && e.servants < 3; k++) spawnServantOf(e, 'demon_eye', ecx, ecy);
          }
        }
        ...
        if (e.def.look === 'golem') {
          ...
          if (e.atkTimer <= 0 && p && !p.dead &&
            ...
            if (TC.Combat && typeof TC.Combat.shockwave === 'function') {
              TC.Combat.shockwave(ecx, e.y + e.h, 3.2 * TC.CONST.TS, e.def.dmg, 260);
            }
          }
        }
        return true;
      ...
      case 'harpy': {
        ...
        if (e.astate === 'dive') {
          e.diveLeft -= dt;
          if (e.diveLeft <= 0) { e.astate = 'idle'; e.atkTimer = rand(2.5, 4); }
          return true;
        }
        ...
        const hsp = Math.hypot(e.vx, e.vy);
        if (hsp > 130) { e.vx *= 130 / hsp; e.vy *= 130 / hsp; }
        e.facing = e.vx >= 0 ? 1 : -1;
        if (e.atkTimer <= 0 && p && !p.dead) { e.astate = 'telegraph'; e.teleTimer = 0.45; }
        return true;
      }
        ...
        if (!e.phase2 && e.hp <= e.maxHp * 0.5) {   // rage at half health
          ...
          if (TC.Particles) {
            TC.Particles.burst(ecx, ecy, 24, {
              colors: [e.def.color, '#ffffff'], speed: 160, life: 0.7, size: 3, gravity: 300
            });
          }
        }
        if (e.onGround) {
      ...
      case 'skeletron': {
        ...
        const parts = e.parts || [];
        let handsAlive = 0;
        for (let k = 0; k < parts.length; k++) if (parts[k] && parts[k].hp > 0) handsAlive++;
        ...
        if (!e.phase2 && (e.hp <= e.maxHp * 0.5 || handsAlive === 0)) {
          ...
          if (TC.Particles) {
            TC.Particles.burst(ecx, ecy, 26, {
              colors: [e.def.color, '#ffd24a'], speed: 170, life: 0.7, size: 3, gravity: 0
            });
          }
        }
        ...
        const hy = pcy - 11 * TC.CONST.TS + Math.sin(clock * 1.1 + e.phase) * 10 - e.h / 2;
        const dx = hx - e.x, dy = hy - e.y;
        const d = Math.hypot(dx, dy) || 1;
        e.vx += (dx / d) * 230 * dt;
        e.vy += (dy / d) * 230 * dt;
        ...
        if (hsp > hmax) { e.vx *= hmax / hsp; e.vy *= hmax / hsp; }
        e.facing = pcx >= ecx ? 1 : -1;
        if (e.dashTimer <= 0 && p && !p.dead) { e.bstate = 'telegraph'; e.teleTimer = 0.45; }
        return true;
      }
      ...
      case 'skele_hand': {
        ...
        if (e.astate === 'lunge') {
          e.lungeLeft -= dt;
          if (e.lungeLeft <= 0) { e.astate = 'orbit'; e.atkTimer = rand(2.2, 3.6); }
          return true;
        }
        const side = e.side || 1;
        ...
        e.facing = e.vx >= 0 ? 1 : -1;
        if (e.atkTimer <= 0 && p && !p.dead) { e.astate = 'windup'; e.teleTimer = 0.4; }
        return true;
      }
      ...
      case 'wof': {
        ...
        if (p && !p.dead) {                   // trail the player's height band
        ...
        e.facing = e.dir;
        if (p && !p.dead) {                   // shed hungries, max 3 alive
          e.summonTimer -= dt;
          if (e.summonTimer <= 0) {
            e.summonTimer = rand(6, 8);
            if (e.servants < 3) spawnServantOf(e, 'hungry', ecx + rand(-20, 20), ecy + rand(-40, 40));
          }
        }
        return true;
      }
  ...
  function killEnemy(e) {
    ...
    if (TC.Particles) {
      if (e.def.boss) {
        TC.Particles.burst(cx, cy, 60, {
          ...
          speed: 260, life: 1.1, size: 4, gravity: 220
        });
      } else {
        TC.Particles.burst(cx, cy, 14, {
          colors: [e.def.color, '#ffffff'],
          speed: 110, life: 0.55, size: 3, gravity: 650
        });
      }
    }
      ...
      for (let k = TC.Enemies.list.length - 1; k >= 0; k--) {
        ...
        if (s !== e && s.master === e) {
          if (TC.Particles && typeof TC.Particles.burst === 'function') {
            try { TC.Particles.burst(s.x + s.w / 2, s.y + s.h / 2, 8, { colors: [s.def.color, '#ffffff'], speed: 90, life: 0.4, size: 2.5, gravity: 500 }); } catch (err) {}
          }
    ...
    if (bloodMoon && !e.def.boss && !e.def.part &&
        ...
        TC.Items && typeof TC.Items.spawnDrop === 'function') {
      TC.Items.spawnDrop(cx, cy, 'blood_shard', 1 + (Math.random() < 0.4 ? 1 : 0), true);
    }
  ...
  function spawnDirector(dt) {
    ...
    const p = TC.player, w = TC.world;
    if (!p || p.dead || !w) { spawnTimer = 0.5; return; }

    const C = TC.CONST, ts = C.TS;
    const dl = daylight();
    ...
    for (let attempt = 0; attempt < 14; attempt++) {
      ...
      const ty = clamp(Math.round(prow + Math.sin(ang) * dist), 1, w.height - 2);
      const spot = findSpot(def, tx, ty, surf);
      if (spot) { list.push(makeEnemy(type, def, spot.x, spot.y)); return; }
    }
  }
  ...
  function spawnServantOf(boss, type, bx, by) {
    ...
    if (!def) return null;
    for (let t = 0; t < 6; t++) {
      const x = bx + rand(-56, 24), y = by + rand(-56, 24);
      if (rectSolid(x, y, def.w, def.h)) continue;
      ...
      if (TC.Particles) {
        TC.Particles.burst(x + def.w / 2, y + def.h / 2, 8, {
          colors: [def.color, '#ffffff'], speed: 90, life: 0.4, size: 2, gravity: 0
        });
      }
      return s;
    }
  ...
  function spawnBoss(type, x, y) {
    if (type === '__blood_moon__') { setBloodMoon(true); return null; }
    const def = TC.ENEMY_DEFS ? TC.ENEMY_DEFS[type] : null;
    if (!def || !def.boss) return null;
    let bosses = 0;
    for (let i = 0; i < list.length; i++) if (list[i].def && list[i].def.boss) bosses++;
    if (bosses >= (TC.CONST.MAX_BOSSES || 1)) return null;
    const p = TC.player;
    if (typeof x !== 'number' || !isFinite(x)) x = p ? p.x + p.w / 2 - def.w / 2 : 0;
    if (typeof y !== 'number' || !isFinite(y)) y = p ? p.y - def.h - 14 * TC.CONST.TS : 0;
    ...
    if (type === 'king_slime') {
      ...
      for (let s = -1; s <= 1; s += 2) {
        ...
        const h = makeEnemy('skele_hand', hdef, x + s * (def.w / 2 + hdef.w), y + 10);
        h.master = e;
        h.side = s;
        e.parts.push(h);
        list.push(h);
    ...
    if (TC.Particles) {
      TC.Particles.burst(x + def.w / 2, y + def.h / 2, 30, {
        colors: [def.color, '#ffffff'], speed: 200, life: 0.8, size: 3, gravity: 0
      });
    }
  ...
  function update(dt) {
    ...
    for (let i = list.length - 1; i >= 0; i--) {
      ...
      if (runAI(e, dt) === false) { list.splice(i, 1); continue; }
      ...
      // contact damage
      if (p && !p.dead && e.fade >= 1 && e.touchTimer <= 0 &&
          e.x < p.x + p.w && e.x + e.w > p.x &&
          e.y < p.y + p.h && e.y + e.h > p.y &&
          TC.Combat && typeof TC.Combat.hurtPlayer === 'function') {
        const dir = (p.x + p.w / 2) >= (e.x + e.w / 2) ? 1 : -1;
        TC.Combat.hurtPlayer(e.def.dmg, dir * TOUCH_KB_X, TOUCH_KB_Y, e.def.name);
  ...
  function drawSlime(c, e) {
    ...
    const k = e.onGround ? Math.sin(clock * 2.2 + e.phase) * 0.05
  ...
  function drawZombie(c, e) {
    ...
    c.fillStyle = skin;
    c.fillRect(f === 1 ? x + w * 0.42 : x - w * 0.85 + w * 0.43, y + h * 0.33, w * 0.85, h * 0.12);
  ...
  function drawEye(c, e) {
    ...
    const ir = r * 0.55;
    const icx = cx + tx * r * 0.34, icy = cy + ty * r * 0.34;
  ...
  function drawBat(c, e) {
    ...
    for (let s = -1; s <= 1; s += 2) {
      ...
      c.quadraticCurveTo(cx + s * bw * 0.9, cy - bh * (0.55 + flap * 0.4), tipX, tipY);
      c.quadraticCurveTo(cx + s * bw * 1.05, cy + bh * (0.1 - flap * 0.25),
                         cx + s * bw * 0.68, cy + bh * (0.02 - flap * 0.3));
      c.quadraticCurveTo(cx + s * bw * 0.46, cy + bh * (0.5 - flap * 0.15),
                         cx + s * bw * 0.18, cy + bh * 0.3);
      c.closePath();
      c.fill();
    }
  ...
  function drawEyeBoss(c, e) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const dx = bx * ca - by * sa, dy = bx * sa + by * ca;  // fanned back dir
    ...
    for (let i = 0; i < 7; i++) {
      ...
      let rr = r * 0.97, aa = (i / 7) * Math.PI * 2 + e.phase * 0.13;
      c.beginPath();
    ...
    const ir = r * 0.52;
    const icx = cx + tx * r * 0.3, icy = cy + ty * r * 0.3;
    if (p2) {   // phase 2: pulsing red glow behind the iris
    ...
    c.fillStyle = '#1c060c';
    c.beginPath();
    c.arc(icx + tx * ir * 0.28, icy + ty * ir * 0.28, ir * 0.48, 0, Math.PI * 2);
    c.fill();
    c.fillStyle = 'rgba(255,255,255,0.75)';
    c.fillRect(icx - ir * 0.5, icy - ir * 0.55, 3, 3);

    if (p2) {   // faint enraged wash over the whole eye
  ...
  function drawWalker(c, e) {
    ...
    if (look === 'skeleton') {
      ...
      c.lineWidth = 2;
      c.beginPath();
      c.moveTo(x + w * 0.35, y + h * 0.55); c.lineTo(x + w * 0.30 + step * 3, y + h);
      c.moveTo(x + w * 0.65, y + h * 0.55); c.lineTo(x + w * 0.70 - step * 3, y + h);
      c.moveTo(x + w * 0.5, y + h * 0.22); c.lineTo(x + w * 0.5, y + h * 0.58);
      c.moveTo(x + w * 0.5, y + h * 0.30);
      c.lineTo(x + w * (f === 1 ? 0.95 : 0.05), y + h * 0.42 + step * 2);
      for (let i = 0; i < 3; i++) {
        const ry = y + h * (0.30 + i * 0.09);
        c.moveTo(x + w * 0.28, ry); c.lineTo(x + w * 0.72, ry);
      }
      c.stroke();
      c.fillStyle = '#e4ddca';
      ...
      c.fillRect(x + (f === 1 ? w * 0.66 : w * 0.40), y + h * 0.09, 3, 3);
      c.fillStyle = '#b8b2a0';
      c.fillRect(x + w * 0.34, y + h * 0.17, w * 0.32, 2);
    } else if (look === 'golem') {
      // bulky stone body with a pulsing energy core
      c.fillStyle = '#5a6874';
      c.fillRect(x + w * 0.10, y + h * 0.25, w * 0.80, h * 0.45);
      c.fillRect(x + w * 0.06 + step * 2, y + h * 0.30, w * 0.20, h * 0.34);
      c.fillRect(x + w * 0.74 - step * 2, y + h * 0.30, w * 0.20, h * 0.34);
      c.fillStyle = '#7a8a96';
      ...
      c.stroke();
      c.fillStyle = hexA('#8adfff', 0.55 + Math.sin(clock * 4 + e.phase) * 0.25);
      c.fillRect(x + w * 0.42, y + h * 0.38, w * 0.16, h * 0.14);
      c.fillStyle = '#3a4a56';
      c.fillRect(x + (f === 1 ? w * 0.58 : w * 0.26), y + h * 0.09, 4, 3);
    } else {   // crawler: low, wide blood spider
      ...
      c.fill();
      c.fillStyle = '#ffd24a';
      c.fillRect(x + w * (f === 1 ? 0.62 : 0.26), y + h * 0.30, 3, 3);
  ...
  function drawHarpy(c, e) {
    ...
    for (let s = -1; s <= 1; s += 2) {
      ...
      c.quadraticCurveTo(cx + s * bw * 1.1, cy - bh * (0.7 + flap * 0.5), tipX, tipY);
      c.quadraticCurveTo(cx + s * bw * 1.0, cy + bh * (0.2 - flap * 0.3),
                         cx + s * bw * 0.5, cy + bh * 0.15);
      c.closePath();
      c.fill();
    }
    ...
    c.fillStyle = e.def.color;
    c.beginPath();
    c.ellipse(cx, cy, bw * 0.55, bh * 0.75, diving ? f * 0.5 : 0, 0, Math.PI * 2);
    c.fill();
  ...
  function drawSkeletron(c, e) {
    ...
    c.quadraticCurveTo(cx + r * 0.82, cy + r * 0.42, cx + r * 0.55, cy + r * 0.52);
    c.lineTo(cx - r * 0.55, cy + r * 0.52);
    c.quadraticCurveTo(cx - r * 0.82, cy + r * 0.42, cx - r * 0.82, cy - r * 0.12);
    c.closePath();
    c.fill();

    // eye sockets glow red once enraged
    c.fillStyle = p2 ? hexA('#ff2038', 0.75 + Math.sin(clock * 6) * 0.2) : '#1c1814';
    c.beginPath();
    c.ellipse(cx - r * 0.36, cy - r * 0.16, r * 0.22, r * 0.27, 0, 0, Math.PI * 2);
    c.ellipse(cx + r * 0.36, cy - r * 0.16, r * 0.22, r * 0.27, 0, 0, Math.PI * 2);
    c.fill();
  ...
  function drawWof(c, e) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      c.moveTo(x + 4, vy);
      c.quadraticCurveTo(x + w * 0.5, vy + Math.sin(clock * 3 + i) * 6, x + w - 4, vy + 4);
    }
    c.stroke();

    // central eye tracks the player
    const ecx = x + w / 2, ecy = y + h * 0.32, er = w * 0.3;
    let tx = e.dir || 1, ty = 0;
    const p = TC.player;
    if (p) {
      const dx = p.x + p.w / 2 - ecx, dy = p.y + p.h / 2 - ecy;
      const d = Math.hypot(dx, dy) || 1;
      tx = dx / d; ty = dy / d;
    }
    c.fillStyle = '#e8ddce';
    ...
    c.beginPath();
    c.arc(ecx + tx * er * 0.34, ecy + ty * er * 0.34, er * 0.52, 0, Math.PI * 2);
    c.fill();
    c.fillStyle = '#200a0a';
    c.beginPath();
    c.arc(ecx + tx * er * 0.48, ecy + ty * er * 0.48, er * 0.24, 0, Math.PI * 2);
    c.fill();

    // toothy maw
    const my = y + h * 0.58, mw = w * 0.7, mh = h * 0.16;
    c.fillStyle = '#4a0f0f';
    c.fillRect(x + w * 0.15, my, mw, mh);
    c.fillStyle = '#e8ddce';
    for (let i = 0; i < 5; i++) {
      const tw = mw / 5, mx = x + w * 0.15 + i * tw;
  ...
  function drawHungry(c, e) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      c.fillRect(cx + Math.cos(a) * r * 0.62 - 1.5, cy + Math.sin(a) * r * 0.62 - 1.5, 3, 3);
    }
  }
  ...
  function draw(ctx, cam) {
    ...
    for (let i = 0; i < list.length; i++) {
      const e = list[i];
      if (vw && (e.x + e.w < vx0 || e.x > vx1 || e.y + e.h < vy0 || e.y > vy1)) continue;
      ctx.save();
      ...
      else if (ai === 'bat') drawBat(ctx, e);          // vulture reuses this too
  ...
  TC.Enemies = {
    list, update, draw, spawnDirector, clear, damageEnemy, spawnBoss,
    isBloodMoon: function () { return bloodMoon; },
