...
(function () {
  ...
  function extendTables() {
    ...
    if (T.WIRE == null) {
      ...
      defs.push(
        ...
        /* SWITCH_OFF     */ mkDef("switch", {
          hardness: 0.08,
          tool: "any",
          drop: "switch",
          pattern: "switch",
          needsSupport: "any",
          colors: ["#8a5a32", "#c0c0cc"],
        }),
        /* SWITCH_ON      */ mkDef("switch (on)", {
          hardness: 0.08,
          tool: "any",
          drop: "switch",
          pattern: "switch",
        ...
        /* LEVER_OFF      */ mkDef("lever", {
          hardness: 0.08,
          tool: "any",
          drop: "lever",
          pattern: "lever",
          needsSupport: "any",
          colors: ["#6a6a74", "#c0c0cc"],
        }),
        /* LEVER_ON       */ mkDef("lever (on)", {
          hardness: 0.08,
          tool: "any",
          drop: "lever",
          pattern: "lever",
        ...
        /* PRESSURE_PLATE */ mkDef("pressure plate", {
          hardness: 0.05,
          tool: "any",
          drop: "pressure_plate",
          pattern: "plate",
        ...
        /* TIMER          */ mkDef("timer", {
          ...
          pattern: "timer",
          needsSupport: "below",
          colors: ["#4a4a52", "#ffd24a"],
        }),
        /* DART_TRAP      */ mkDef("dart trap", {
          solid: true,
          hardness: 0.35,
          tool: "any",
          drop: "dart_trap",
    ...
    if (TC.ITEM_DEFS && !TC.ITEM_DEFS.wire) {
      const I = (name, kind, o) =>
        Object.assign({ name, kind, maxStack: 999 }, o);
      ...
      TC.ITEM_DEFS.pressure_plate = I("Pressure Plate", "block", {
        tile: T.PRESSURE_PLATE,
      });
    ...
    if (TC.RECIPES && !TC.RECIPES.some((r) => r && r.out === "wire")) {
      TC.RECIPES.push(
        ...
        },
        {
          ...
          n: 1,
          station: "workbench",
          cost: { iron_bar: 3, glass: 1, wire: 2 },
        },
        {
          out: "dart_trap",
          n: 1,
          station: "anvil",
          cost: { iron_bar: 5, stone: 5, wire: 2 },
        },
        {
          out: "actuator",
          n: 3,
  ...
  function spawnDrop(x, y, id, count) {
    if (TC.Items && typeof TC.Items.spawnDrop === "function") {
      try {
        TC.Items.spawnDrop(x, y, id, count);
      } catch (e) {}
    }
  }
  ...
  function fireReceiver(x, y, id) {
    ...
    emitEvent(
      TC.Events && TC.Events.EVENT ? TC.Events.EVENT.WirePulse : "WirePulse",
      { x: x, y: y },
    );
  ...
  function updatePlates() {
    ...
    plates.forEach((st, i) => {
      ...
      for (let k = 0; k < ents.length; k++) {
        const e = ents[k];
        if (aabb(e.x, e.y, e.w, e.h, rx, ry, TS, TS)) {
          hit = true;
          break;
        }
      }
  ...
  function hitEnemies(d, di) {
    if (!TC.Enemies || !TC.Enemies.list || !TC.Enemies.damageEnemy)
      return false;
  ...
  function attachActuator(player, m) {
    ...
    const TSv = TS;
    const tx = Math.floor(m.worldX / TSv),
      ty = Math.floor(m.worldY / TSv);
    if (typeof player.inReach === "function" && !player.inReach(tx, ty))
      return false;
    const id = world.get(tx, ty);
    ...
    if (
      ...
      (typeof inv.count === "function" && !(inv.count("actuator") > 0))
    )
      return false;
    try {
      inv.remove("actuator", 1);
    } catch (e) {
      return false;
    }
    actuated.add(i);
  ...
  function validIdxArray(arr, maxIdx) {
    ...
    for (let i = 0; i < arr.length; i++) {
      const v = arr[i];
      if (typeof v !== "number" || (v | 0) !== v || v < 0 || v >= maxIdx)
        return false;
    }
    return true;
  }
  ...
  function load(blob) {
    ...
    const maxIdx = world
      ...
      : TC.CONST.WORLD_W * TC.CONST.WORLD_H;
    ...
    if (blob.timers != null) {
      ...
      for (let i = 0; i < blob.timers.length; i++) {
        if (timers.has(blob.timers[i]))
          timers.get(blob.timers[i]).running = true;
      }
    }
    if (blob.actuators != null) {
      if (!validIdxArray(blob.actuators, maxIdx)) return false;
      for (let i = 0; i < blob.actuators.length; i++)
        actuated.add(blob.actuators[i]);
    }
    if (blob.ghosts != null) {
  ...
  function strokeLinks(ctx, world, tx, ty, cx, cy) {
    ...
    if (
      tx + 1 < world.width &&
      world.tiles[ty * world.width + tx + 1] === T.WIRE
    ) {
      ctx.moveTo(cx, cy);
    ...
    if (
      ty + 1 < world.height &&
      world.tiles[(ty + 1) * world.width + tx] === T.WIRE
    ) {
      ctx.moveTo(cx, cy);
  ...
  function drawTimer(ctx, px, py, st) {
    ...
    const ang = st
      ? (st.t / TIMER_PERIOD) * Math.PI * 2 - Math.PI / 2
      : -Math.PI / 2;
    ctx.strokeStyle = C.wireHot;
    ctx.lineWidth = 1;
  ...
  function drawActuatorMarks(ctx, world, i) {
    ...
    if (ghosts.has(i)) {
      ...
      // idle corner brackets
      ctx.strokeStyle = C.act;
      ctx.lineWidth = 1;
      const b = 4;
      ctx.beginPath();
      ctx.moveTo(px + 1, py + 1 + b);
      ctx.lineTo(px + 1, py + 1);
      ctx.lineTo(px + 1 + b, py + 1);
      ctx.moveTo(px + TS - 1 - b, py + 1);
      ctx.lineTo(px + TS - 1, py + 1);
      ctx.lineTo(px + TS - 1, py + 1 + b);
      ctx.moveTo(px + TS - 1, py + TS - 1 - b);
      ctx.lineTo(px + TS - 1, py + TS - 1);
      ctx.lineTo(px + TS - 1 - b, py + TS - 1);
      ctx.moveTo(px + 1 + b, py + TS - 1);
      ctx.lineTo(px + 1, py + TS - 1);
      ctx.lineTo(px + 1, py + TS - 1 - b);
      ctx.stroke();
    }
  }
  ...
  function draw(ctx, cam) {
    ...
    const vw = ctx.canvas.width / cam.zoom,
      vh = ctx.canvas.height / cam.zoom;
    ...
    if (actuated.size) {
      // few entries; filter by view
      actuated.forEach((i) => {
        const tx = i % W,
          ty = (i / W) | 0;
        if (tx >= tx0 && tx <= tx1 && ty >= ty0 && ty <= ty1)
          drawActuatorMarks(ctx, world, i);
      });
    }
    drawDarts(ctx);
    ctx.restore();
  ...
  function defineRegistryContent() {
    ...
    const safe = (kind, id, def) => {
      if (!def) return;
      try {
        R.define(kind, id, def);
      } catch (e) {
        /* dup or invalid: non-fatal */
      }
    };
    ...
    const safeItem = (key, id) => {
      ...
      try {
        if (typeof R.aliasKey === "function") R.aliasKey("item", id, key);
      } catch (e2) {}
    };
    const defs = TC.TILE_DEFS;
    if (defs) {
  ...
  function registerSaveProvider() {
    ...
    try {
      ...
      console.warn(
        "[TC.Wiring] SaveCore provider registration skipped:",
        e && e.message,
      );
    }
  }
  ...
  // wiring.js loads AFTER main.js's boot-time syncFromTables and appends
  // recipes/tiles/items to the shared tables — absorb them NOW so registry
  // state (and its fingerprint) is stable from script-load time onward and
  // later syncs are true no-ops.
  if (TC.Registry && typeof TC.Registry.syncFromTables === 'function') {
    try {
      TC.Registry.syncFromTables();
    } catch (e) {
      /* non-fatal: recorded by the registry for validate() */
    }
  }
